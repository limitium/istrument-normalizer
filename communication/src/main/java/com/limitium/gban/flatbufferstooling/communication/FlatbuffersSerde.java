package com.limitium.gban.flatbufferstooling.communication;
...
public class FlatbuffersSerde<T extends Table> implements Serde<T> {
    ...
    public FlatbuffersSerde(Class<T> clazz) {
        String factoryMethodName = "getRootAs" + clazz.getSimpleName();
        try {
            rootMaker = clazz.getMethod(factoryMethodName, ByteBuffer.class);
        } catch (NoSuchMethodException e) {
            throw new RuntimeException("Missing factory method '" + factoryMethodName + "'", e);
        }
    }

...
public class WrappableIndexedKeyValueStoreBuilder<K, V, W, PC extends ProcessorContext> extends IndexedKeyValueStoreBuilder<K, WrapperValue<W, V>> {

    private final WrapperSupplierFactory<K, V, W, PC> wrapperSupplierFactory;

    private final KeyValueBytesStoreSupplier storeSupplier;

    public WrappableIndexedKeyValueStoreBuilder(KeyValueBytesStoreSupplier storeSupplier, Serde<K> keySerde, Serde<V> valueSerde, Serde<W> wrapperSerde, WrapperSupplierFactory<K, V, W, PC> wrapperSupplierFactory, Time time) {
    ...
    @Override
    public WrappableIndexedMeteredKeyValueStore<K, V, W, PC> build() {
        return new WrappableIndexedMeteredKeyValueStore<>(
                getIndexGenerators(),
                nonUniqIndexes,
